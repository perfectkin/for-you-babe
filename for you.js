...
function playMusic() {
  ...
  if (audio) {
    ...
    audio.play()
      .then(() => {
        if (status) {
          status.textContent = 'Music is playing. Enjoy!';
        }
      })
      .catch(() => {
        if (status) {
          status.textContent = 'The phone browser blocked playback. Try tapping the button again or using a different browser.';
        }
      });
  }
}
